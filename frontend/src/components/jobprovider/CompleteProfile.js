...
function CompleteProfile(props) {
    ...
    const handleSave = async (e) => {
        ...
        const response = await fetch(process.env.REACT_APP_BACKEND_URL + "/api/jobprovider/createprofile", {
            method: 'POST',
            headers: {
    ...
    useEffect(async () => {
        const response = await fetch(process.env.REACT_APP_BACKEND_URL + "/api/jobprovider/getemail", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'auth-token': localStorage.getItem('token')
