...
function CreateProfile(props) {
    ...
    const createProfile = async (e) => {
        ...
        const response = await fetch(process.env.REACT_APP_BACKEND_URL + "/api/jobseeker", {
            method: 'POST',
            headers: {
    ...
    useEffect(async () => {
        const response = await fetch(process.env.REACT_APP_BACKEND_URL + "/api/jobseeker/getemail", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'auth-token': localStorage.getItem('token')
